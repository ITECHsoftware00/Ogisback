...
export default function CreatorWithdraw() {
  ...
  const feeAmt = selectedMethod ? Math.round(parseFloat(amount || 0) * selectedMethod.fee / 100 * 100) / 100 : 0;
  const netAmount = Math.round((parseFloat(amount || 0) - feeAmt) * 100) / 100;
  ...
  if (success && lastWithdrawal) return (
    <DashboardLayout>
      ...
      <div className="max-w-lg mx-auto py-16 text-center">
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-20 h-20 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center mx-auto mb-6">
          <CheckCircle size={40} className="text-green-600" />
        </motion.div>
        <h1 className="text-2xl font-heading font-bold text-gray-900 dark:text-white mb-2">Withdrawal Initiated!</h1>
        <p className="text-gray-500 mb-2">Your withdrawal of <strong>{formatCurrency(lastWithdrawal.netAmount)}</strong> via <strong>{lastWithdrawal.methodName}</strong> has been submitted.</p>
        <p className="text-gray-400 text-sm mb-8">Funds will arrive based on your chosen method's processing time. Status: <span className="font-semibold text-yellow-600">Pending review</span></p>
        <button onClick={() => { setSuccess(false); setAmount(''); setAccountInfo(''); setMethod(''); }} className="btn btn-outline btn-lg">Make Another Withdrawal</button>
      </div>
    </DashboardLayout>
  );
    ...
    <DashboardLayout>
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="rounded-2xl bg-gradient-to-br from-wallet to-orange-400 text-white p-6 mb-6">
          ...
          <p className="text-white/60 text-xs mt-2">Minimum withdrawal: $10</p>
        </div>
        ...
        <div className="card p-6 mb-4 space-y-4">
          <div className="form-group">
            ...
            <div className="relative">
              ...
              <input type="number" value={amount} onChange={e => setAmount(e.target.value)} max={walletBalance} min={10} placeholder="0.00" className="input pl-8 text-lg font-semibold" />
            </div>
            <div className="flex gap-2 mt-2">
              {[25, 50, 100].map(pct => (
                <button key={pct} onClick={() => setAmount(Math.round(walletBalance * pct / 100).toString())} className="text-xs px-2.5 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-wallet/10 hover:text-wallet transition-all">
                  {pct}%
                </button>
              ))}
              <button onClick={() => setAmount(walletBalance.toString())} className="text-xs px-2.5 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-wallet/10 hover:text-wallet transition-all">Max</button>
            </div>
          </div>
          ...
          {method && (
            <div className="form-group">
              <label className="label">
                {method === 'paypal' ? 'PayPal Email' : method === 'bank' ? 'Bank Account Number / IBAN' : method === 'wise' ? 'Wise Email or Account ID' : 'Account Details'}
              </label>
          ...
          {amount && parseFloat(amount) > 0 && method && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 space-y-2 text-sm">
              ...
              {feeAmt > 0 && <div className="flex justify-between"><span className="text-gray-500">Service fee ({selectedMethod?.fee}%)</span><span className="text-red-500">-{formatCurrency(feeAmt)}</span></div>}
        ...
        <div className="flex items-center gap-2 mb-4 text-xs text-gray-400">
          <Shield size={13} /> All withdrawals are processed securely and reviewed within 24h.
        </div>

        <button onClick={handleWithdraw} disabled={processing || !method || !amount || parseFloat(amount) < 10} className="btn btn-lg w-full disabled:opacity-50" style={{ background: 'linear-gradient(135deg,#F59E0B,#EF4444)', color: 'white' }}>
          {processing
            ...
            : <><ArrowDownToLine size={18} />Withdraw {amount ? formatCurrency(netAmount) : 'Now'}</>
          }
        </button>
        ...
        {history.length > 0 && (
          <div className="mt-8 card p-6">
            ...
            <div className="space-y-3">
              {history.map(w => (
                <div key={w.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-all">
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm ${w.status === 'completed' ? 'bg-green-100 dark:bg-green-900/20' : w.status === 'failed' ? 'bg-red-100 dark:bg-red-900/20' : 'bg-yellow-100 dark:bg-yellow-900/20'}`}>
                  ...
                  <div className="text-right">
                    ...
                    <span className={`badge text-[10px] ${w.status === 'completed' ? 'badge-success' : w.status === 'failed' ? 'badge-danger' : 'badge-warning'}`}>{w.status}</span>
                  </div>
                </div>
              ))}
            </div>
