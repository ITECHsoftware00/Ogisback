...
function ConvRow({ conv, index }) {
  const name   = conv.creator_profiles?.name     || 'Creator';
  const avatar = conv.creator_profiles?.avatar_url || `https://i.pravatar.cc/48?u=${conv.creator_id}`;
  ...
  return (
    ...
    >
      ...
      >
        {unread > 0 && (
          <span className="absolute left-0 top-4 bottom-4 w-[3px] rounded-r-full bg-brand" />
        )}
        <div className="relative flex-shrink-0">
          <img src={avatar} alt="" className="w-11 h-11 rounded-2xl object-cover" />
          {unread > 0 && (
            <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-brand rounded-full border-2 border-white dark:border-[#111118]" />
          )}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline justify-between gap-2 mb-0.5">
            ...
            <p className={`text-[11px] flex-shrink-0 ${unread > 0 ? 'text-brand font-semibold' : 'text-gray-400'}`}>
        ...
        {unread > 0 ? (
          <motion.span
            ...
            className="flex-shrink-0 min-w-[20px] h-5 bg-brand text-white text-[10px] font-bold rounded-full flex items-center justify-center px-1.5 shadow-sm"
          >
            {unread}
          </motion.span>
        ) : (
...
export default function BrandMessages() {
  ...
  const filtered    = convs.filter(c => {
    const name = c.creator_profiles?.name || '';
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            className="w-full bg-gray-50 dark:bg-white/[0.04] border border-gray-100 dark:border-white/[0.07] rounded-2xl pl-10 pr-9 py-2.5 text-sm text-gray-900 dark:text-white placeholder-gray-400 outline-none focus:ring-2 focus:ring-brand/25 focus:border-brand/30 transition-all"
          />
          {search && (
            ...
            {unread.length > 0 && (
              <>
                <div className="px-4 pt-3 pb-1.5">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-brand">Unread</p>
                </div>
