          ...
          {filtered.map((conv, i) => {
            ...
            return (
              <motion.div key={conv.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
                <Link to={`/brand/messages/${conv.id}`} className={`flex items-center gap-4 p-4 rounded-2xl border transition-all hover:shadow-card ${unread > 0 ? 'bg-white dark:bg-[#111118] border-brand/20 shadow-sm' : 'bg-white dark:bg-[#111118] border-gray-100 dark:border-gray-800'}`}>
                  <div className="relative">
                    <img src={avatar} alt="" className="w-12 h-12 rounded-2xl object-cover" />
                    {unread > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-brand rounded-full text-white text-[10px] font-bold flex items-center justify-center">{unread}</span>}
                  </div>
