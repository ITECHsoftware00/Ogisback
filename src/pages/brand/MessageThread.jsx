...
import { ArrowLeft, Send, Paperclip, Reply, Trash2, X, Zap } from 'lucide-react';
...
export default function BrandMessageThread() {
  ...
  const send = useCallback(async () => {
    ...
    setMsgs(prev => [...prev, {
      id: tempId, body: text, sender_id: user.id, sender_role: 'brand',
          ...
          {loading ? (
            ...
            <>
              <div className="relative">
                <img src={creatorAvatar} alt="" className="w-10 h-10 rounded-2xl object-cover" />
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <p className="font-heading font-semibold text-gray-900 dark:text-white leading-tight">{creatorName}</p>
                  <span className="text-[10px] font-semibold text-gray-400 bg-gray-100 dark:bg-white/[0.07] px-1.5 py-0.5 rounded-full">Creator</span>
                </div>
                <p className="text-xs text-green-500 font-medium">Active now</p>
              </div>
              <Link
                to={`/brand/discover/${conv?.creator_profiles?.username}`}
                className="btn btn-brand btn-sm gap-1.5 flex-shrink-0"
              >
                <Zap size={13} /> Hire
              </Link>
            </>
            ...
            return (
              <div key={msg.id}>
                ...
                <motion.div
                  ...
                  className={`flex items-end gap-2 ${isMe ? 'justify-end' : 'justify-start'} ${isLast ? 'mb-2' : 'mb-0.5'}`}
                >
                  {!isMe && (
                    <div className="w-7 flex-shrink-0">
                      {isLast && <img src={creatorAvatar} alt="" className="w-7 h-7 rounded-full object-cover" />}
                    </div>
                  )}
                  ...
                  <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[72%]`}>
                    ...
                    <div className={`px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-wrap break-words transition-opacity
                      ${isMe
                        ? `bg-gradient-brand text-white ${msg._pending ? 'opacity-50' : 'opacity-100'}
                  ...
                  {/* Actions for other person's messages */}
                  {!isMe && isHover && !msg._pending && (
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => setReplyTo({ id: msg.id, body: msg.body, senderRole: msg.sender_role })}
        ...
        <div className="pt-3 border-t border-gray-100 dark:border-white/[0.06]">
            ...
            {replyTo && (
              ...
              >
                <div className="flex items-center gap-2 px-3 py-2 mb-2 bg-gray-50 dark:bg-white/[0.04] border border-gray-100 dark:border-white/[0.07] rounded-xl text-xs text-gray-500">
                  <Reply size={12} className="text-brand flex-shrink-0" />
                  <span className="flex-1 truncate">
                    <span className="font-semibold text-brand mr-1">
                      {replyTo.senderRole === 'brand' ? 'You' : creatorName}
                    </span>
                    {replyTo.body}
                  </span>
          ...
          <div className="flex items-end gap-2 bg-gray-50 dark:bg-white/[0.04] border border-gray-100 dark:border-white/[0.07] rounded-2xl px-3 py-2">
            ...
            <textarea
              ...
              onKeyDown={handleKey}
              placeholder={replyTo ? 'Write a reply…' : `Message ${creatorName}…`}
              rows={1}
            ...
            <AnimatePresence>
              {input.trim() && (
                <motion.button
                  ...
                  onClick={send}
                  className="w-8 h-8 rounded-xl bg-brand text-white flex items-center justify-center hover:opacity-90 transition-opacity flex-shrink-0 self-end mb-0.5"
                >
                  <Send size={15} />
                </motion.button>
              )}
          ...
          <p className="text-[10px] text-gray-400 mt-1.5 text-center">Enter to send · Shift+Enter for new line · Esc to cancel reply</p>
        </div>
